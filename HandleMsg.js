...
const apakah = [
        ...
        const seberapagay = [
                '100%\n*É melhor você trocar as cartas em vez de levar um tiro 1 por 1*',
                '95%\n*Milos Detected*',
                '90%',
                '85%',
                '80%',
...
module.exports = HandleMsg = async (aruga, message) => {
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'Funciona converter etiqueta em imagem!', id)
                .then(() => {
	...
	case 'cooltext':
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, 'Aqui\'é o seu adesivo')
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'Aqui\'é o seu adesivo ')).then(() => console.log(`Adesivo processado para ${processTime(t, moment())} Second`))
            } else {
                await aruga.reply(from, `Sem imagens! Usar ${prefix}sticker\n\n\nEnvie fotos com legendas\n${prefix}sticker <devjava>\n${prefix}sticker nobg <sem fundo>\n\nem enviar mensagem com\n${prefix}sticker <link da imagem>`, id)
            }
            break
            case 'antisticker':
            case 'antistiker':
                    ...
                    if (args[0] == 'on') {
                        ...
                        if(cek){
                            return aruga.reply(from, '*Detector de adesivos anti-spam* já ativo neste grupo', id) //if number already exists on database
                        } else {
                            antisticker.push(chatId)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM]* foi ativado \ nCada membro do grupo cujo adesivo de spam for maior que 7 será chutado pelo bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        ...
                        aruga.reply(from, `selecione / fora\n\n*[Anti Sticker SPAM]*\nCada membro do grupo que é um adesivo de spam será chutado pelo bot!`, id)
                    }
                    break
                    case 'antilink':
	...
	case 'brainly':
            ...
            if (args.length >= 2){
			    ...
			    aruga.sendText(from, 'Feito ✅, se increva aqui https://youtube.com/channel/UCE7x81dLhF0BQSDLSqCUjBQ | Instagram : https://instagram.com/devjavastudios?igshid=6kwwzixyg5jp')
                        } else {
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    aruga.reply(from, `[❗] Envie um GIF com uma legenda *${prefix}stickergif* máx. 10 seg!`, id)
                   }
                } else {
      ...
      break
        case 'ttp':
                ...
                try
                {
                    ...
                    if(args)
                    {
                        if(quotedMsgObj == null)
                        {
                            ...
                            if(gasMake.status == true)
                            {
                                try{
                                    ...
                                    await aruga.reply(from, 'Falha ao criar.', id)
                                } 
                            }else{
                        ...
                        await aruga.reply(from, 'Não pode estar vazio.', id)
                    }
                }catch(error)
                {
                    console.log(error)
        ...
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                await aruga.reply(from, `Sem imagem! Envie uma foto com uma legenda ${prefix}meme <text_up> | <text_under>\nexemplo: ${prefix}meme texto superior | texto abaixo`, id)
            }
            break
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, `Uso ${prefix}quotmaker | citação isi | autor | tema\n\nexemplo: ${prefix}quotemaker | Eu te amo | -devjava | aleatório\n\nuntuk O tema é uso aleatório, irmão ..`)
            }
            break
        case 'apakah':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
            ...
            if (!nanya) aruga.reply(from, '⚠️ Formato incorreto! Digite * #menu * para usar.')
            await aruga.sendText(from, `Questão: *${nanya}* \n\nResponda: ${responda}`)
            break
        case 'bisakah':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando só pode ser usado em grupos!', id)
            const bsk = args.join(' ')
            const jbsk = bisakah[Math.floor(Math.random() * (bisakah.length))]
            if (!bsk) aruga.reply(from, '⚠️ Formato incorreto! Digite * # menu * para usar.')
            await aruga.sendText(from, `Questão: *${bsk}* \n\nResponda: ${jbsk}`)
            break
        case 'kapankah':
        ...
        case "mirip": 
	      ...
	      aruga.reply(from, `${name} mirip dengan ${mirip[random]}`, id); 
           break
        case 'blackpink':
            if (args.length === 1) return aruga.reply(from, `Enviar pedidos *#blackpink [ Texto ]*, contoh *#blackpink Benni*`, id)
            tobz.reply(from, mess.wait, id)
            const blpk = body.slice(11)
            if (blpk.length > 10) return aruga.reply(from, '*Texto muito longo!*\n_Máximo de 10 letras!_', id)
            await aruga.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${blpk}&apikey=${vhtearkey}`, 'blackpink.jpg', '', id)
            break
        case 'thunder':
            if (args.length === 1) return aruga.reply(from, `Enviar pedidos *#thunder [ Texto ]*, contoh *#thunder Benni*`, id)
            ...
            break
	case 'ttp2':
            if (args.length === 1) return aruga.reply(from, `Enviar pedidos *#ttp2 [ Teks ]*, contoh *#ttp2 Benni*`, id)
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'este é o resultado, então não seja preguiçoso, é por isso', id)
            .catch(() => {
                aruga.reply(from, 'Algo deu errado!', id)
            })
            break
         case 'magernulis1':
                if (args.length === 4) return await aruga.reply(from, 'Enviar Comando */nulis2 --[nome]--[classe]--[texto]*\n\n*Exemplo :*\n/nulis2 --Benniismael--7B--Se inscrever DEV JAVA', id)
                arg = body.trim().split('--')
                const diNama = arg[1]
                const diKelas = arg[2]
                const diTulis = arg[3]
	...
	//Group All User
	case 'grouplink':
            if (!isBotGroupAdmins) return aruga.reply(from, 'Este comando só pode ser usado quando o bot se torna admin ', id)
            if (isGroupMsg) {
                const inviteLink = await aruga.getGroupInviteLink(groupId);
                aruga.sendLinkWithAutoPreview(from, inviteLink, `\nLink group *${name}* Usar *${prefix}revogar* reiniciar Link group`)
            } else {
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
                    if (isBotGroupAdmins) {
                        aruga
                            ...
                            .then((res) => {
                                aruga.reply(from, `Use com sucesso Revogar Link do Grupo *${prefix}linkgrup* para obter o último link de convite de grupo`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        case 'tiktok':
            ...
            try {
                ...
                aruga.sendText(ownerNumber, 'Erro de download de Tiktok : '+ err)
        ...
        case 'cachorro':
            ...
            aruga.sendFileFromUrl(from, kya, 'Dog.jpeg', 'Dog ✨️', id)
            break
        case 'gato':
                    await aruga.simulateTyping(from, true)
                    aruga.sendText(from, '⏳ Espere, querida, no processo de gato. . . ⏳')
        ...
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                aruga.reply(from, `Desculpe, a consulta não está disponível. Por favor digite ${prefix}kpop para ver a lista de consulta`)
            }
            break
        case 'memes':
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
		...
		case 'mutegrup':
            ...
            if (args[0] == 'on') {
				aruga.setGroupToAdminsOnly(groupId, true).then(() => aruga.sendText(from, 'Alterado com sucesso para que apenas administradores possam bater papo membros comum choram🌝!'))
			} else if (args[0] == 'off') {
		...
		case 'setprofile':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro do grupo!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione o bot como administrador do grupop!', id)
        ...
        if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione o bot como administrador do grupo!', id)
            ...
            aruga.reply(from, 'Sucesso remover todos os membros', id)
        break

        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Este pedido é apenas para proprietários de LightBot!', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *LightBot  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *LightBot  B C* 〙\n\n${msg}`)
            }
            aruga.reply(from, 'Broadcast Success!', id)
            break
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para proprietários de LightBot', id)
        ...
        case 'clearall': //menghapus seluruh pesan diakun bot
            if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para proprietários do bots ', id)
